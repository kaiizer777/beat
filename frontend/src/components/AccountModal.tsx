...
export function AccountModal({ isOpen, onClose }: AccountModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl shadow-cyan-950/30 text-slate-100 relative overflow-hidden grid grid-rows-[minmax(0,1fr)_auto] max-h-[85vh]"
      ...
      >
        ...
        {/* Actions */}
        <div className="flex items-center justify-between p-6 pt-4 border-t border-slate-800 bg-slate-900/50">
          <button
            onClick={() => signOut()}
