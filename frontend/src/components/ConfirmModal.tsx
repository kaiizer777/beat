    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
        <button
          onClick={onCancel}
          disabled={isLoading}
          className="absolute right-4 top-4 text-slate-400 hover:text-white transition disabled:opacity-50"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="mt-6 flex justify-end space-x-3">
          <button
            ...
            onClick={onCancel}
            disabled={isLoading}
            className="rounded-lg border border-slate-700 bg-slate-800/80 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-700 transition focus:outline-none disabled:opacity-50"
          >
            {cancelLabel}
          </button>
          <button
