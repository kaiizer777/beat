...
- [ ] 14.3 Decide and implement how the backend itself runs: since it's no longer a permanently-running process waiting on a `TaskScheduler`, it needs to be reachable on-demand when GitHub Actions calls it. Use a free serverless-friendly host for the Spring Boot app reachable over HTTPS with no card required — confirm current no-card options (e.g. Fly.io's free allowances, Koyeb's free web service tier) against today's actual terms before picking one, since these change; do not assume a specific one without verifying it's still card-free at build time. Document whichever is chosen and why, right in this file, once decided.
- [ ] 14.4 Write the GitHub Actions workflow (`.github/workflows/run-digests.yml`): `on.schedule.cron: '*/5 * * * *'`, one job that does a simple authenticated `curl`/HTTP call to the `run-due-channels` endpoint from 14.1, passing the shared secret from a GitHub repo secret (never hardcoded in the YAML).
- [ ] 14.5 Add all required secrets to the GitHub repo under Settings → Secrets and variables → Actions: the shared internal secret from 14.1, and confirm the backend host from 14.3 already has its own env vars (`DATABASE_URL`, `TINYFISH_API_KEY`, `GROQ_API_KEY`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `AUTH_SECRET`) set on its own platform, not duplicated into GitHub.
