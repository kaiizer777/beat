...
**Goal:** Take the now feature-complete, audited, multi-user, polished app live for real — the deferred deployment steps from `work.md` Phase 1, done properly this time, plus Phase 9's reliability hardening if not already done.

- [ ] 14.1 Provision the Oracle Cloud Always Free VM (`work.md` 1.4, deferred at the time) — Ampere A1 Flex, with the documented capacity-error fallback to the AMD Micro shape if needed. Install Java 21, open port 8080 at both the Security List/NSG and local firewall layers.
- [ ] 14.2 Build the production JAR and deploy it to the VM as a proper `systemd` service from the start (skip the manual `nohup` shortcut entirely this time — go straight to what `work.md` 9.1 eventually required) — auto-restart on crash, starts on VM reboot.
- [ ] 14.3 Set all backend env vars on the VM: `DATABASE_URL`, `TINYFISH_API_KEY`, `GROQ_API_KEY`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `AUTH_SECRET` (must match the frontend's), plus anything else added during Phase 10/11 JWT setup. Confirm none are hardcoded anywhere in source.
- [ ] 14.4 Deploy the frontend to Vercel (connect the GitHub repo). Set all frontend env vars: `NEXT_PUBLIC_API_BASE_URL` (pointing at the VM's public IP/domain), `AUTH_SECRET`, `AUTH_RESEND_KEY`, `DATABASE_URL` (for the Prisma/Auth.js adapter), and any Auth.js-specific vars from Phase 10.
- [ ] 14.5 Update CORS on the Spring Boot backend (`work.md` 7.6) to allow only the real production Vercel domain, not `localhost` or `*`.
- [ ] 14.6 Full production smoke test: from the live Vercel URL, sign in with a real email, create a channel, let it fire on schedule (or manually trigger), confirm the digest email arrives, confirm the reading view works — the same full-loop test as `audit.md` Phase C, but against production infrastructure this time.
- [ ] 14.7 Reboot the Oracle VM once, confirm the `systemd` service comes back up automatically and channel schedules resume firing without any manual restart — final proof this survives unattended for real use.
- [ ] 14.8 Update the project `README.md` with the real production URLs (frontend + any relevant info) and a short "for friends: how to sign up and use it" section.

**Checkpoint:** Beat is live at a real public URL, a friend can sign up and use it without your involvement, the VM survives a reboot unattended, and everything built across both `work.md` and this file is verified working in production, not just on the dev server.
